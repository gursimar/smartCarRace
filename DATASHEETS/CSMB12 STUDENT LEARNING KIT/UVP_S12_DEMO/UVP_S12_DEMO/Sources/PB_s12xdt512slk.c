...
void PBMCUSLK_PB_Init(void)
{
  ...
  DDRA_DDRA4 = 0;   //Enable as Input (PB3)
  DDRA_DDRA5 = 0;   //Enable as Input (PB4)      
...
void PBMCUSLK_LED_Init(void)
{
  ...
  DDRA_DDRA0 = 1;  //Enable as Output (LED4)
  DDRA_DDRA1 = 1;  //Enable as Output (LED3)
  DDRA_DDRA2 = 1;  //Enable as Output (LED2)
  DDRA_DDRA3 = 1;  //Enable as Output (LED1)
  PUCR_PUPAE = 1;  //Enable Pull Resistors for PORT A
  /* Insert user code here to initialize UNDEFINED PBMCUSLK LEDs */
}
/************************************************************************************************/

/* Initialization routine to configure pins as outputs to write LEDx level. */
void APSLK_LED_Init(void)
{  
  DDRB_DDRB4 = 1;  //Enable as Output (LED4)
  DDRB_DDRB5 = 1;  //Enable as Output (LED3)
  DDRB_DDRB6 = 1;  //Enable as Output (LED2)
  DDRB_DDRB7 = 1;  //Enable as Output (LED1)
  PUCR_PUPAE = 1;  //Enable Pull Resistors for PORT B
}
/************************************************************************************************/
...
void PBMCUSLK_POT_Init(void)
{
  ...
  ATD0CTL5 |= (ATD0CTL5_SCAN_MASK |				//ATD scan mode enabled
              ATD0CTL5_CA_MASK);					//ATD single channel scan of ATD Ch.1
