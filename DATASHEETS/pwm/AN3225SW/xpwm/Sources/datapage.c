...
/*--------------------------- _FAR_COPY_ HCS12X Routines --------------------------------
  ...
  There are the following memory block copy routines:
      ...
      _FAR_COPY:                HC12/HCS12 struct copy routine.
      ...
      _FAR_COPY_GLOBAL_GLOBAL:
      _FAR_COPY_GLOBAL_LOGICAL:
      _FAR_COPY_LOGICAL_GLOBAL:
      _FAR_COPY_LOGICAL_LOGICAL:
      _FAR_COPY_NEAR_GLOBAL:
      _FAR_COPY_NEAR_LOGICAL:
      _FAR_COPY_GLOBAL_NEAR:
      _FAR_COPY_LOGICAL_NEAR:    HCS12X specific far copy routine. The name describes what the src/dest address format are.
                                 ...
                                 (Note: HC12 _FAR_COPY is using X/A as src and Y/B as dest, so the register usage is not the same!)

  Arguments :
  - B:X : src address (for NEAR: only X)
  - A:Y : dest address (for NEAR: only Y)
  - number of bytes to be copied at 2,SP. The number of bytes is always > 0
...
void NEAR _FAR_COPY_GLOBAL_GLOBAL(void) {
  __asm {
        PSHA                      ;// push dest page
Loop:
        STAB    __GPAGE_ADR__
        GLDAA   1,X+
        MOVB    0,SP,__GPAGE_ADR__
        GSTAA   1,Y+
        DECW    3,SP
        BNE     Loop
        PULA                      ;// clean stack
...
void NEAR _SET_PAGE_REG_HCS12X(void) {
  __asm {
  /* Moves the page stored in B into the page register controlling the area X */
        // 0000..FFFF
        CPX     #0x8000
        BHS     _GT8000
        // 0000..7FFF
...
_GT8000:
        ...
        // 8000..BFFF
        STAB    __PPAGE_ADR__
_NON_PAGED:                       ;// NON PAGED (0000..07FF || 0C00..0FFF || 2000..7FFF || C000..FFFF) or fall tbrough from 8000..BFFF
        RTS
  }
}
#endif
...
void NEAR _FAR_COPY_GLOBAL_LOGICAL(void) {
  __asm {
        STAB    __GPAGE_ADR__
        EXG     X,Y
        TFR     A,B
...
void NEAR _FAR_COPY_LOGICAL_LOGICAL(void) {
  __asm {
        PSHA
        __PIC_JSR(_CONV_LOGICAL_TO_GLOBAL);
        PULA
        __PIC_JMP(_FAR_COPY_GLOBAL_LOGICAL);
  }
}
...
void NEAR _FAR_COPY_NEAR_GLOBAL(void) {
  __asm {
        CLRB
        __PIC_JMP(_FAR_COPY_LOGICAL_GLOBAL);
  }
}
...
void NEAR _FAR_COPY_NEAR_LOGICAL(void) {
  __asm {
        PSHA
...
void NEAR _FAR_COPY_GLOBAL_NEAR(void) {
  __asm {
...
void NEAR _FAR_COPY_LOGICAL_NEAR(void) {
  __asm {
        EXG     A,B
        EXG     X,Y
        PSHA
